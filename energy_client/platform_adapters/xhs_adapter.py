...
with the Xiaohongshu (XHS) platform for signature generation and login.
...
_build_sign_string = build_sign_string
...
class XHSEnergyAdapter:
    ...
    async def get_qrcode_image(self) -> bytes:
        ...
        Returns:
            Base64 decoded QR code image bytes
        """
        ...
        if not result:
            ...
            (function() {
                const btn = document.querySelector('button.login-btn') ||
                           document.evaluate("//button[contains(text(), '登录')]", document, null,
            ...
            """
            self._execute_js_raw(click_script)
    ...
    async def check_login_status(self, no_logged_in_session: str) -> bool:
        """
        Check if user has successfully logged in.

        Args:
            no_logged_in_session: Session ID before login

        Returns:
            True if logged in, False otherwise
        """
        check_profile_script = """
        (function() {
            const profileLink = document.querySelector('a[href*="/user/profile/"]');
            if (profileLink) {
                const meSpan = profileLink.querySelector('span');
                if (meSpan && meSpan.textContent.includes('我')) {
                    return true;
                }
            }
            return false;
        ...
        """
        result = self._execute_js_raw(check_profile_script)
        if result and result.lower() == 'true':
            return True

        # Check if web_session cookie changed
    ...
    async def login_with_cookies(self, cookies: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if cookie_list:
                return self.browser.set_cookies(self.browser_id, cookie_list)
            return False
        except Exception:
            return False

    async def click_login_button(self) -> bool:
        """Click the login button to open login dialog."""
        script = """
        (function() {
            const loginBtn = document.querySelector('#app button') ||
                            document.evaluate("//button[contains(text(), '登录')]",
                                            document, null,
    ...
    async def switch_to_phone_login(self) -> bool:
        """Switch to phone login method in the login dialog."""
        await asyncio.sleep(1)
        script = """
        (function() {
            const phoneLoginTab = document.querySelector('.login-container .other-method div:first-child') ||
                                 document.evaluate("//div[contains(@class, 'other-method')]//div[contains(text(), '手机')]",
                                                 document, null,
                                                 XPathResult.FIRST_ORDERED_NODE_TYPE,
                                                 null).singleNodeValue;
            if (phoneLoginTab) {
    ...
    async def fill_phone_number(self, phone: str) -> bool:
        """Fill in phone number for login."""
        await asyncio.sleep(0.5)
        script = f"""
        (function() {{
            const container = document.querySelector('.login-container');
            if (container) {{
                const phoneInput = container.querySelector('label.phone > input');
                if (phoneInput) {{
    ...
    async def send_verification_code(self) -> bool:
        ...
        (function() {
            const container = document.querySelector('.login-container');
            if (container) {
                const sendBtn = container.querySelector('label.auth-code > span');
                if (sendBtn) {
                    sendBtn.click();
                    return true;
                }
    ...
    async def fill_verification_code(self, code: str) -> bool:
        ...
        (function() {{
            const container = document.querySelector('.login-container');
            if (container) {{
                const codeInput = container.querySelector('label.auth-code > input');
                if (codeInput) {{
    ...
    async def agree_privacy_policy(self) -> bool:
        """Click to agree to privacy policy."""
        await asyncio.sleep(0.5)
        script = """
        (function() {
            const privacyCheckbox = document.querySelector('.agreements svg') ||
                                   document.evaluate("//div[contains(@class, 'agreements')]//*[local-name()='svg']",
                                                   document, null,
                                                   XPathResult.FIRST_ORDERED_NODE_TYPE,
                                                   null).singleNodeValue;
            if (privacyCheckbox) {
    ...
    async def submit_login(self) -> bool:
        ...
        (function() {
            const container = document.querySelector('.login-container');
            if (container) {
                const submitBtn = container.querySelector('.input-container > button');
                if (submitBtn) {
                    submitBtn.click();
                    return true;
                }
