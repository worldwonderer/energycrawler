...
# Repository: https://github.com/EnergyCrawler/EnergyCrawler/blob/main/store/xhs/__init__.py
...
async def update_xhs_note(note_item: Dict):
    ...
    local_db_item = {
        ...
        "last_modify_ts": utils.get_current_timestamp(),  # Last modification timestamp (Generated by EnergyCrawler, mainly used to record the latest update time of a record in DB storage)
...
async def update_xhs_note_sub_comment(note_id: str, root_comment_id: str, sub_comment: Dict):
    """
    Update Xiaohongshu note sub-comment.
    Reuses first-level comment storage schema with parent_comment_id bound to root comment id.
    """
    if not sub_comment:
        return

    target_comment = dict(sub_comment.get("target_comment") or {})
    if not target_comment.get("id"):
        target_comment["id"] = root_comment_id

    normalized_comment = dict(sub_comment)
    normalized_comment["target_comment"] = target_comment
    normalized_comment.setdefault("sub_comment_count", 0)
    await update_xhs_note_comment(note_id, normalized_comment)
...
async def save_creator(user_id: str, creator: Dict):
    ...
    local_db_item = {
        ...
        'tag_list': json.dumps({tag.get('tagType'): tag.get('name')
                                for tag in tags}, ensure_ascii=False),  # Tags
        "last_modify_ts": utils.get_current_timestamp(),  # Last modification timestamp (Generated by EnergyCrawler, mainly used to record the latest update time of a record in DB storage)
    }
